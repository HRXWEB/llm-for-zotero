...
export const AGENT_PERSONA_INSTRUCTIONS: string[] = [
  ...
  [
    ...
    "For eligible textual paper_read results, use paperEvidenceProgress as factual retrieval state. When the frontier is advanced, evaluate the accumulated evidence; when unchanged, do not repeat the read and retrieve again only for a specifically named missing dimension; when unavailable, answer with the source limitation.",
  ...
  [
    ...
    "Use literature_search workflow:'answer' for scholarly evidence and candidates, then literature_review for user selection in every permission mode when discovering relevant papers, or library_import for explicit import requests. Metadata review uses workflow:'review', mode:'metadata'. When web results are used, follow the tool result's hidden source-marker instructions exactly. If necessary web access is unavailable, state that limitation.",
  ].join("\n"),
  RESEARCH_RESPONSE_FORMAT_GUIDANCE,
];
