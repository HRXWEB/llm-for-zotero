...
export function buildSemanticPrompt(
  ...
  return [
    ...
    'Action constraints contain only tagPrefix:string, readMode:"full", or collectionMode:"move". Omit irrelevant constraints. Add-only filing preserves existing memberships by default and has no collectionMode. Other restrictions use decisions.constraints.',
    "A collection scope.path is an exact literal name by default. Preserve every word in a supplied collection name; never shorten it to a shared suffix. Only for a genuinely descriptive reference, such as a collection described by its purpose rather than named, set scope.referenceKind to descriptive. A missing literal name is unresolved; a similar catalog name is not authority to substitute a different destination.",
    ...
    '{"schemaVersion":1,"taskKind":"read|write|mixed","queryLanguage":"en","requestedScopes":["none|single-paper|paper-set|library-corpus|note|visual-input"],"selections":[{"skillId":"existing skill ID","requestedScope":"single-paper","evidenceText":"exact user text","occurrence":0}],"retrievalIntent":"enumerate|verify|summarize|none","deliverableIntent":"chat|document|unspecified","paperTargetIntent":"active|added|all_visible|unspecified","externalSearchIntent":"none|web|literature|both","wantedSections":[],"writeDisposition":"none|required|uncertain","actionIntents":[],"decisions":{"constraints":[],"noteDestination":"none|zotero|file|both","conversationOnly":false,"reading":{"source":"metadata|document_text|rendered_pages","coverage":"overview|targeted|exhaustive"},"literature":"none|discover|import|select_then_import","bulk":false,"continuation":"new|resume|revise","questions":[]}}',
    `Every actionIntents entry MUST use this complete JSON Schema. Include the reference fields needed for the requested action; the empty envelope example is not permission to omit required actions or their destinations. Optional means absent when irrelevant, not absent when the user supplied the value: ${JSON.stringify(ACTION_INTENT_RESPONSE_SCHEMA)}`,
    ...
    "Optional action fields are scope, parameters, constraints, targetSelectors, discovery, dependsOn, and contentFrom as specified above. Omit targetSelectors when context supplies the target. Omit irrelevant fields entirely: never emit discovery:{}, empty selectors, null references, or fabricated parameters. For document deliverables include documentKind from its enumerated values.",
